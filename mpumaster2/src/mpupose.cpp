...
{
    int ball_model_index = getIndex(model_states.name, "barbaros");
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "mpupose");

  ros::NodeHandle private_node_handle("~");
  private_node_handle.param<std::string>("port", port, "/dev/ttyUSB0");
  private_node_handle.param<std::string>("tf_parent_frame_id", tf_parent_frame_id, "barbaros/base_link");
  private_node_handle.param<std::string>("tf_frame_id", tf_frame_id, "barbaros/imu_link");
  private_node_handle.param<std::string>("frame_id", frame_id, "barbaros/imu_link");
  ...
  ros::Subscriber link_states_subscriber = nh.subscribe("/gazebo/link_states", 100, link_states_callback);
          ...
          while (input.length() >= 28) // while there might be a complete package in input
          {
            ...
            if (data_packet_start != std::string::npos)
            {
              ...
              if ((input.length() >= data_packet_start + 28) && (input.compare(data_packet_start + 26, 2, "\r\n") == 0))  //check if positions 26,27 exist, then test values
              {
                ...
                model_pose.orientation.x = imu.orientation.x;
                model_pose.orientation.y = imu.orientation.y;
                model_pose.orientation.z = imu.orientation.z;
                model_pose.orientation.w = imu.orientation.w;
                ...
                set_model_state("barbaros", "world", model_pose, model_twist);
